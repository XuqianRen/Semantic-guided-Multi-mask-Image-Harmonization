...
#             nn.Linear(opt.ksize*opt.ksize*(opt.input_nc), dim)
#         )
#         self.patch_to_embedding_mask = nn.Sequential(
#             Rearrange('b c (h p1) (w p2) -> b (h w) (p1 p2 c)', p1 = opt.ksize, p2 = opt.ksize),
#             nn.Linear(opt.ksize*opt.ksize*(1), dim)
#         )

#         self.transformer_enc = transformer.TransformerEncoders(2*dim, nhead=opt.tr_r_enc_head, num_encoder_layers=opt.tr_r_enc_layers, dim_feedforward=dim*opt.dim_forward, activation=opt.tr_act)
#         # self.transformer_enc.eval()
#         self.dec = nn.Sequential(
#             nn.ConvTranspose2d(2*dim, opt.output_nc, kernel_size=opt.ksize, stride=opt.ksize, padding=0),
#             nn.Tanh()
#         )

#     def forward(self, comp, mask, pixel_pos=None):
#         patch_embedding = self.patch_to_embedding(comp) # [b,4,256,256] -> [b,4096,256] 4x4个patch
#         patch_embedding_mask = self.patch_to_embedding_mask(mask)
#         patch_embedding_mask = torch.new_ones(patch_embedding_mask)
#         patch_embedding = torch.cat([patch_embedding, patch_embedding_mask],dim=2)

#         pixel_pos_mask = pixel_pos

#         pixel_pos = torch.cat([pixel_pos,pixel_pos_mask],dim=2)

#         content = self.transformer_enc(patch_embedding.permute(1, 0, 2), src_pos=pixel_pos) # [4096,1,256]
#         bs, L, C  = patch_embedding_mask.size()  


#         harmonized = self.dec(content.permute(1,2,0).reshape(bs, 2*C, int(math.sqrt(L)), int(math.sqrt(L)))) #[1,256,64,64] --> [1,3,256,256]
#         return harmonized
...
class HTGenerator(nn.Module):
    ...
    def forward(self, inputs, pixel_pos=None):
        ...
        harmonized_add = self.dec1(content.permute(1,2,0).view(bs, C, int(math.sqrt(L)), int(math.sqrt(L))))
        harmonized_mul = self.dec2(content.permute(1,2,0).view(bs, C, int(math.sqrt(L)), int(math.sqrt(L))))
        harmonized = torch.cat([harmonized_add,harmonized_mul],axis=1)
        return harmonized
...
class DHTGenerator(nn.Module):
    def __init__(self, opt=None):
        ...
        self.reflectance_maskdec = ContentDecoder(opt.n_downsample, 0, self.reflectance_enc.output_dim, opt.output_nc, opt.ngf, 'ln', opt.activ, pad_type=opt.pad_type)
        self.reflectance_dec = ContentDecoder(opt.n_downsample, 0, self.reflectance_enc.output_dim, 3, opt.ngf, 'ln', opt.activ, pad_type=opt.pad_type)
        ...
        self.color_generator = GlobalLighting(light_element=opt.light_element,light_mlp_dim=self.reflectance_dim, opt=opt)
        self.color_render= transformer.TransformerDecoders(self.reflectance_dim, nhead=opt.tr_i_dec_head, num_decoder_layers=opt.tr_i_dec_layers, dim_feedforward=self.reflectance_dim*opt.dim_forward, activation=opt.tr_act)
        self.color_dec = ContentDecoder(opt.n_downsample, 0, self.reflectance_dim, opt.output_nc, opt.ngf, 'ln', opt.activ, pad_type=opt.pad_type)
        
        self.contrast_generator = GlobalLighting(light_element=opt.light_element,light_mlp_dim=self.reflectance_dim, opt=opt)
        self.contrast_render= transformer.TransformerDecoders(self.reflectance_dim, nhead=opt.tr_i_dec_head, num_decoder_layers=opt.tr_i_dec_layers, dim_feedforward=self.reflectance_dim*opt.dim_forward, activation=opt.tr_act)
        self.contrast_dec = ContentDecoder(opt.n_downsample, 0, self.reflectance_dim, opt.output_nc, opt.ngf, 'ln', opt.activ, pad_type=opt.pad_type)
        
        
        self.opt = opt
    def forward(self, inputs=None, image=None,  pixel_pos=None, patch_pos=None, mask_r=None, mask=None, layers=[], encode_only=False):
        r_content = self.reflectance_enc(inputs) #ER [b,256,64,64]
        # bs,c,h,w = inputs.size()
        bs,c,h,w = r_content.size()
        #TRER
        reflectance = self.reflectance_transformer_enc(r_content.flatten(2).permute(2, 0, 1), src_pos=pixel_pos, src_key_padding_mask=None) # [4096,b,256]

        #DR
        pre_mask = reflectance.permute(1, 2, 0).view(bs, c, h, w)      
        pre_mask = self.reflectance_maskdec(pre_mask) #[b,1,256,256]
        pre_mask = pre_mask/2+0.5
        mask = torch.where(pre_mask>0.5, torch.ones_like(pre_mask),torch.zeros_like(pre_mask)).cuda()
        reverse_mask = 1-mask
        image = inputs*reverse_mask
        ...
        #TREL+TRDL
        color_code, color_embed = self.color_generator(image[:,0,:,:].unsqueeze(1), pos=patch_pos, mask=mask, use_mask=self.opt.light_use_mask) # [27,b,256],[27,b,256]        
        #TRDL
        color = self.color_render(color_code, reflectance, src_pos=color_embed, tgt_pos=pixel_pos, src_key_padding_mask=None, tgt_key_padding_mask=None) # [4096,b,256]
        #DI
        color = color.permute(1, 2, 0).view(bs, c, h, w)
        color = self.color_dec(color)*mask #[b,3,256,256]




        #TREL+TRDL
        light_code, light_embed = self.light_generator(image[:,1,:,:].unsqueeze(1), pos=patch_pos, mask=mask, use_mask=self.opt.light_use_mask) # [27,b,256],[27,b,256]        
        #TRDL
        illumination = self.illumination_render(light_code, reflectance, src_pos=light_embed, tgt_pos=pixel_pos, src_key_padding_mask=None, tgt_key_padding_mask=None) # [4096,b,256]
        #DI
        illumination = illumination.permute(1, 2, 0).view(bs, c, h, w)
        illumination = self.illumination_dec(illumination)*mask #[b,3,256,256]


        #TREL+TRDL
        contrast_code, contrast_embed = self.contrast_generator(image[:,2,:,:].unsqueeze(1), pos=patch_pos, mask=mask, use_mask=self.opt.light_use_mask) # [27,b,256],[27,b,256]        
        #TRDL
        contrast = self.contrast_render(contrast_code, reflectance, src_pos=contrast_embed, tgt_pos=pixel_pos, src_key_padding_mask=None, tgt_key_padding_mask=None) # [4096,b,256]
        #DI
        contrast = contrast.permute(1, 2, 0).view(bs, c, h, w)
        contrast = self.contrast_dec(contrast)*mask #[b,3,256,256]


        reflectance = reflectance.permute(1, 2, 0).view(bs, c, h, w)      
        reflectance = self.reflectance_dec(reflectance) #[b,1,256,256]
        reflectance = reflectance/2+0.5




        h, l, s = reflectance.split(1, dim = 1)
        h = h + (color) 
        l = l + (illumination)
        s = s + (contrast)
        ...
        harmonized = torch.cat([h, l, s], dim = 1)
       
        # harmonized = reflectance*illumination
        return harmonized, pre_mask, mask, color, illumination, contrast
