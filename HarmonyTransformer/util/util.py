...
def tensor2im(input_image, imtype=np.uint8):
    ...
    if not isinstance(input_image, np.ndarray):
        ...
        if image_numpy.shape[0] == 1:  # grayscale to RGB
            image_numpy = np.tile(image_numpy* 255, (3, 1, 1))
            image_numpy = (np.transpose(image_numpy, (1, 2, 0)) + 0) / 1.0 
        # image_numpy = (np.transpose(image_numpy, (1, 2, 0)) + 1) / 2.0 * 255.0  # post-processing: tranpose and scaling
        else:
            image_scale = [255.0,255.0,255.0]
            image_numpy = (np.transpose(image_numpy, (1, 2, 0)) + 0) / 1.0 * image_scale  # post-processing: tranpose and scaling
    ...
    # if label!='mask':
    #     image_numpy = cv2.cvtColor(image_numpy, cv2.COLOR_LAB2RGB)
...
def save_image(image_numpy, image_path, label, aspect_ratio=1.0):
